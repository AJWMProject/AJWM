...
*Setup instructions for a Gitian build of Ajwm Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ajwm
...
- [Building Ajwm Core](#building-ajwm-core)
...
Unixy OSes by entering the following in a terminal:
    ...
    echo "ad4e8c27c561ad8248d5ajwm1d36eb172f884057bfeb2c22ead823f59fa8c3dff  debian-8.5.0-amd64-netinst.iso" | sha256sum -c
    # (must return OK)
...
Clone the git repositories for ajwm and Gitian.
...
```bash
sudo cp /home/debian/ajwm/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the ajwm repository under 'Fetch and create inputs' to install sources which require
...
To build Ajwm Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ajwm repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ajwm/.git/
...
For example:
```bash
URL=https://github.com/laanwj/ajwm.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ajwm=${COMMIT} --url ajwm=${URL} ../ajwm/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ajwm=${COMMIT} --url ajwm=${URL} ../ajwm/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ajwm=${COMMIT} --url ajwm=${URL} ../ajwm/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-ajwm:
./bin/gbuild --commit ajwm=${COMMIT},cpp-eth-ajwm=${ETHCOMMIT} --url ajwm=${URL},cpp-eth-ajwm=${ETHURL} ../ajwm/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the ajwm git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ajwm/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ajwm
SIGPATH=/some/root/path/ajwm-detached-sigs

./bin/gbuild --url ajwm=${BTCPATH},signature=${SIGPATH} ../ajwm/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ajwm-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ajwm-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ajwm-osx-build.assert
```
...
[ajwmproject/gitian.sigs](https://github.com/ajwmproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@ajwm.org) and he will commit them.