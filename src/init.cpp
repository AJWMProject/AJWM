...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ajwmproject/ajwm>";
    const std::string URL_WEBSITE = "<https://ajwm.org>";
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                fs::path ajwmStateDir = GetDataDir() / "stateAjwm";
                bool fStatus = fs::exists(ajwmStateDir);
                const std::string dirAjwm(ajwmStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsAjwmstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<AjwmState>(new AjwmState(dev::u256(0), AjwmState::openDB(dirAjwm, hashDB, dev::WithExisting::Trust), dirAjwm, existsAjwmstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::ajwmMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(ajwmStateDir.string()));
                if (fReset) {
