...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setAjwmSchedule(ajwmDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = ajwmDGP.getBlockSize(nHeight);
    minGasPrice = ajwmDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = ajwmDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    AjwmTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractAjwmTX resultConverter;
    if(!convert.extractionAjwmTransactions(resultConverter)){
    ...
    for(AjwmTransaction ajwmTransaction : ajwmTransactions){
        txGas += ajwmTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + ajwmTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(ajwmTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, ajwmTransactions, hardBlockGasLimit);
...
void StakeAjwms(bool fStake, CWallet *pwallet)
{
