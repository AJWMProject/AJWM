...
#define paternVersion "ajwm-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

AjwmVersionChecker::AjwmVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

AjwmVersionChecker::~AjwmVersionChecker()
{

}

bool AjwmVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> AjwmVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(AJWM_RELEASES)));
    QEventLoop event;
...
Version AjwmVersionChecker::getMaxReleaseVersion()
{
