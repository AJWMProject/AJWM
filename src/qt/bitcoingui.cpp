...
#include <qt/ajwmversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    ajwmVersionChecker = new AjwmVersionChecker(this);

    if(fCheckForUpdates && ajwmVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(AJWM_RELEASES, AJWM_RELEASES);
        QString message(tr("New version of Ajwm wallet is available on the Ajwm source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Ajwm address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ajwm: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Ajwm addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Ajwm addresses"));
    ...
    openAction->setStatusTip(tr("Open a ajwm: URI or payment request"));//AJWM_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Ajwm command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createMenuBar()
{
    ...
    // Configure the menus   지갑 상단 메뉴관련 설정 부분 shjang
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
       ...
       // QList<QAction*> contractActions;   // 스마트 컨트렉트  주석처리 시작 shjang 
       ...
       // appNavigationBar->mapGroup(smartContractAction, contractActions); // 스마트 컨트렉트  주석처리 끝
       // QList<QAction*> tokenActions;     // 토큰 주석처리 시작 shjang
       // tokenActions.append(sendTokenAction);
       // tokenActions.append(receiveTokenAction);
       // tokenActions.append(addTokenAction);  // 토큰 주석처리 끝 shjang
       // appNavigationBar->mapGroup(QRCTokenAction, tokenActions); // 토큰 주석처리 끝 shjang
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Ajwm network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Ajwm"); // default title
