...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ajwm address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("ajwm"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("ajwm://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, ""); // UST: change string length to 7 to correctly parse ust://
//		uri.replace(0, 7, "ust:"); // UST: change string length to 7 to correctly parse ust://  uri no mark
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ajwm.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ajwm (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ajwm (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ajwm.desktop";
    return GetAutostartDir() / strprintf("ajwm-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ajwm\n";
        else
            optionFile << strprintf("Name=Ajwm (%s)\n", chain);
