...
bool ToHash160(const std::string& strAjwmAddress, std::string& strHash160)
{
    CTxDestination ajwmAddress = DecodeDestination(strAjwmAddress);
    if(!IsValidDestination(ajwmAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&ajwmAddress);
    if(keyid){
...
bool ToAjwmAddress(const std::string& strHash160, std::string& strAjwmAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination ajwmAddress = keyid;
    if(IsValidDestination(ajwmAddress)){
        strAjwmAddress = EncodeDestination(ajwmAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToAjwmAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToAjwmAddress(tokenEvent.receiver, tokenEvent.receiver);
