...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // ajwm
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // ajwm
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // ajwm
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // ajwm
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CTxDestination ajwmSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(ajwmSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&ajwmSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ajwm addresses\n"
