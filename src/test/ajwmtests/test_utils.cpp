#include <ajwmtests/test_utils.h>

void initState(){
    boost::filesystem::path pathTemp;		
    pathTemp = fs::temp_directory_path() / strprintf("test_bitcoin_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
    boost::filesystem::create_directories(pathTemp);
    const std::string dirAjwm = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<AjwmState>(new AjwmState(dev::u256(0), AjwmState::openDB(dirAjwm, hashDB, dev::WithExisting::Trust), dirAjwm + "/ajwmDB", dev::eth::BaseState::Empty));
...
AjwmTransaction createAjwmTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    AjwmTransaction txEth;
    if(recipient == dev::Address()){
        txEth = AjwmTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = AjwmTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
