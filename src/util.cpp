...
const char * const BITCOIN_CONF_FILENAME = "ajwm.conf";
const char * const BITCOIN_PID_FILENAME = "ajwmd.pid";
...
bool OpenDebugLog()
{
    ...
    assert(fileoutVM == nullptr); // ajwm
    assert(vMsgsBeforeOpenLog);
    fs::path pathDebug = GetDebugLogPath();
    fs::path pathDebugVM = GetDebugVMLogPath(); // ajwm
    fileout = fsbridge::fopen(pathDebug, "a");
    fileoutVM = fsbridge::fopen(pathDebugVM, "a"); // ajwm
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ajwm
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ajwm
    // Mac: ~/Library/Application Support/Ajwm
    // Unix: ~/.ajwm
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ajwm";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ajwm";
#else
    // Unix
    return pathRet / ".ajwm";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Ajwm Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Ajwm Core developers";
    }
