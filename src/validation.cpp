...
#if defined(NDEBUG)
# error "Ajwm cannot be compiled without assertions."
#endif
...
std::unique_ptr<AjwmState> globalState;
...
const std::string strMessageMagic = "Ajwm Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            AjwmDGP ajwmDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = ajwmDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = ajwmDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            AjwmTxConverter converter(tx, NULL);
            ExtractAjwmTX resultConverter;
            if(!converter.extractionAjwmTransactions(resultConverter)){
            ...
            std::vector<AjwmTransaction> ajwmTransactions = resultConverter.first;
            std::vector<EthTransactionParams> ajwmETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(AjwmTransaction ajwmTransaction : ajwmTransactions){
                sumGas += ajwmTransaction.gas() * ajwmTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, ajwmTransaction.gasPrice());
                } else {
                    txMinGasPrice = ajwmTransaction.gasPrice();
                }
                VersionVM v = ajwmTransaction.getVersion();
                if(v.format!=0)
                ...
                if(ajwmTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(ajwmTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(ajwmTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)ajwmTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(ajwmETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > ajwmTransactions.size())
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s AJWM additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    if(nHeight <= consensusParams.nLastPOWBlock)
        return 15400000 * COIN; //ajwm 20000 * COIN;
    ...
    CAmount nSubsidy = 5 * COIN; //ajwm 3 * COIN;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // ajwm
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // ajwm
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    AjwmTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<AjwmTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path ajwmDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(ajwmDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(ajwmDir.string(), std::ios::in | std::ios::out);
...
bool AjwmTxConverter::extractionAjwmTransactions(ExtractAjwmTX& ajwmtx){
    std::vector<AjwmTransaction> resultTX;
    ...
    ajwmtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool AjwmTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool AjwmTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
AjwmTransaction AjwmTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    AjwmTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = AjwmTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = AjwmTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // ajwm
    AjwmDGP ajwmDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setAjwmSchedule(ajwmDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = ajwmDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = ajwmDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = ajwmDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ajwm
...
///////////////////////////////////////////////////////////////////////////////////////// ajwm
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            AjwmTxConverter convert(tx, &view, &block.vtx);

            ExtractAjwmTX resultConvertAjwmTX;
            if(!convert.extractionAjwmTransactions(resultConvertAjwmTX)){
            ...
            if(!CheckMinGasPrice(resultConvertAjwmTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertAjwmTX.first, blockGasLimit);
            ...
            for(AjwmTransaction& qtx : resultConvertAjwmTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertAjwmTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertAjwmTX.first[k].from(), resultConvertAjwmTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(77000000000 + ((pindex->nHeight - 5000) * 5)) * COIN){ //ajwm (uint64_t)(100000000 + ((pindex->nHeight - 5000) * 4)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // ajwm
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // ajwm
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // ajwm
            globalState->setRootUTXO(oldHashUTXORoot); // ajwm
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // ajwm
        uint32_t sizeBlockDGP = ajwmDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Ajwm PoS we need to sync this after every block to ensure txdb is populated for
