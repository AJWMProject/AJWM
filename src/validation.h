...
/////////////////////////////////////////// ajwm
#include <ajwm/ajwmstate.h>
#include <ajwm/ajwmDGP.h>
#include <libethereum/ChainParams.h>
#include <libethashseal/Ethash.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <ajwm/storageresults.h>
...
using ExtractAjwmTX = std::pair<std::vector<AjwmTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // ajwm
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashAjwmRoot,
    const dev::h256& oldHashStateRoot, const std::vector<AjwmTransaction>& transactions);
...
class AjwmTxConverter{

public:

    AjwmTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionAjwmTransactions(ExtractAjwmTX& ajwmTx);
...
private:
    ...
    AjwmTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<AjwmTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
...
private:
    ...
    std::vector<AjwmTransaction> txs;
